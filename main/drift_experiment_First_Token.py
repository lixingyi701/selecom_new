"""
Post-training Drift Experiment — 策略：FIRST ANSWER CONTENT TOKEN
=================================================================

测量方式：对每个样本，仅在答案第一个实际内容 token 位置计算 KL 散度。

    答案序列结构（以 "Neil Armstrong" 为例）：
      [ '<' 'answer' '>' | Neil  Armstrong | '</' 'answer' '>' '<|im_end|>' ]
        ←─ 前缀(3 tok) ─→   ↑
                        本版本仅测量这一个位置
                        P(Neil | compressed_context)

优点：
  1. 与设计文档公式含义（"答案首 token"）严格对应。
  2. 与 PISCO、OSCAR 等软压缩论文的标准做法一致，结果可横向比较。
  3. 语义最纯粹——此时模型仅凭 soft prompt 预测，无任何 teacher-forced
     答案前缀信息；后续 token 均以已生成内容为条件，不如此处干净。

缺点：单 token 测量的方差较大；对于多词答案，忽略了后续词的分布差异。

与 drift_experiment_All_Token.py 的唯一代码差异：
    本文件：content_labeled = labeled_pos[content_start : content_start+1]  # 仅第一个内容位置
    对比版：content_labeled = labeled_pos[content_start : content_end]       # 全部内容位置

运行示例：
    cd /home/lxy/selecom/main
    python drift_experiment_First_Token.py \\
        ...
        --output_path /home/lxy/selecom/results/drift_results_first_token.json
"""
...
# ---------------------------------------------------------------------------
# [FIRST_TOKEN] 仅在第一个答案内容 token 位置提取 logits
# ---------------------------------------------------------------------------
...
def get_answer_content_logits(generator, proj_embeddings, generator_input_ids,
    ...
    """
    [FIRST_TOKEN 版本]
    仅提取答案第一个实际内容 token 对应的 logit 向量（跳过 <answer> 前缀格式标签）。

    答案序列布局：
      [ '<' 'answer' '>' | tok1 tok2 ... tokM | '</' 'answer' '>' '<|im_end|>' ]
        ←── N_FMT_PREFIX ──→  ↑
                          本函数只取这一个位置
                          = P(tok1 | soft_prompt, question)，无 teacher-forcing 干扰

    返回：list of B tensors，每个形状 [1, vocab_size]。
    """
    ...
    for i in range(B):
        ...
        if content_end > content_start:
            # ★ FIRST_TOKEN：仅取第一个内容 token 位置，形状 [1]
            content_labeled = labeled_pos[content_start : content_start + 1]
        else:
            # 答案极短，格式标签已覆盖全部；回退到第一个 labeled 位置
            content_labeled = labeled_pos[:1]

        pred_positions = (content_labeled - 1).clamp(min=0)
        results.append(logits[i, pred_positions, :].half().cpu())  # [1, vocab]

    return results


# ---------------------------------------------------------------------------
# KL 散度：KL(P1 || P2)，单 token 位置
# ---------------------------------------------------------------------------

def compute_kl_divergence(logits1_list, logits2_list):
    """
    [FIRST_TOKEN 版本]
    对每个样本在第一个内容 token 位置计算 KL(P1||P2)。
    由于每个样本只有 1 个位置，.mean() 等价于直接取该值。

    Args:
        logits1_list: list of N tensors，每个 [1, vocab_size]
    ...
    for l1, l2 in zip(logits1_list, logits2_list):
        ...
        p1     = log_p1.exp()
        kl_per_pos = (p1 * (log_p1 - log_p2)).sum(dim=-1)  # [1]
        kl_samples.append(kl_per_pos.mean())                # 等价于直接取该值
...
def run_experiment(args):
    ...
    print('Measurement strategy: FIRST ANSWER CONTENT TOKEN only')
    ...
    print('\n=== Computing KL Divergences (FIRST_TOKEN) ===')
    ...
    THRESHOLD = 0.5
    print('\n' + '=' * 60)
    print('POST-TRAINING DRIFT RESULTS  [FIRST_TOKEN]')
    print('=' * 60)
    print(f'  Measurement     : KL at FIRST answer content token only')
    print(f'  Samples         : {N}')
    ...
    if args.output_path:
        ...
        with open(args.output_path, 'w', encoding='utf-8') as f:
            json.dump({
                'measurement_strategy': 'FIRST_TOKEN',
                'num_samples': N,
...
def main():
    parser = argparse.ArgumentParser(description='Drift experiment — FIRST_TOKEN strategy')
